...
local BUILTIN = {
  edifact = { segments = {
    ...
    BGM={title="Beginning of message",elements={{name="C002 Document/message name",components={{name="1001 Document name code",id="1001"},{name="1131 Code list ID",id="1131"},{name="3055 Code list agency",id="3055"},{name="1000 Document name",id="1000"}}},{name="C106 Document message ID"},{name="1225 Message function, coded",id="1225"},{name="4343 Response type, coded",id="4343"}}},
    DTM={title="Date/time/period",elements={{name="C507 Date/time/period",components={{name="2005 Qualifier",id="2005"},{name="2380 Date/time/period",id="2380"},{name="2379 Format qualifier",id="2379"}}}}},
    RFF={title="Reference",elements={{name="C506 Reference",components={{name="1153 Reference qualifier",id="1153"},{name="1154 Reference number",id="1154"},{name="1156 Line number",id="1156"},{name="4000 Reference version",id="4000"}}}}},
    NAD={title="Name and address",elements={{name="3035 Party qualifier",id="3035"},{name="C082 Party identification"},{name="C058 Name and address"},{name="C080 Party name"},{name="C059 Street"},{name="3164 City",id="3164"},{name="3251 Postcode",id="3251"},{name="3207 Country",id="3207"}}},
    ERC={title="Application Error Information",elements={{name="C901 Application error detail",components={{name="9321 Application error identification",id="9321"},{name="1131 Code list ID",id="1131"},{name="3055 Agency",id="3055"}}}}},
    FTX={title="Free text"},
    LIN={title="Line item"}, QTY={title="Quantity"}, PRI={title="Price details"}, MOA={title="Monetary amount"},
    UNT={title="Message Trailer"}, UNZ={title="Interchange Trailer"},
  }},
  x12 = { segments = {
    ISA={title="Interchange Control Header"}, GS={title="Functional Group Header"}, ST={title="Transaction Set Header"},
    SE={title="Transaction Set Trailer"}, GE={title="Functional Group Trailer"}, IEA={title="Interchange Control Trailer"},
  }},
}
...
local function build_tree(buf)
  ...
  for i, line in ipairs(lines) do
    ...
    if tag == "UNH" then
      ...
      if mshort then label = "Message "..mshort end
...
local function strtoupper_list(lst)
  ...
  for _,v in ipairs(lst or {}) do
    if type(v) == "string" then table.insert(out, v:upper()) end
  end
  return out
end
...
local function norm_group_obj(g, key_hint)
  ...
  if kids then
    if is_array(kids) then
      local arr = {}
      for _,child in ipairs(kids) do table.insert(arr, norm_group_obj(child)) end
      o.children = arr
    else
      local arr = {}
      for k,child in pairs(kids) do table.insert(arr, norm_group_obj(child, k)) end
      table.sort(arr, function(a,b)
        local na = tonumber((a.id or ""):match("SG(%d+)") or "") or 1e9
        local nb = tonumber((b.id or ""):match("SG(%d+)") or "") or 1e9
        if na == nb then return (a.id or "") < (b.id or "") end
        return na < nb
      end)
      o.children = arr
    end
...
local function normalize_schema(js)
  ...
  if is_array(groups) then
    for _,g in ipairs(groups) do table.insert(out, norm_group_obj(g)) end
  else
    for k,g in pairs(groups) do table.insert(out, norm_group_obj(g, k)) end
...
local function scan_json_candidates(root, stems)
  local want = {}
  for _,st in ipairs(stems) do want[(st..".json"):lower()] = true end
  local h = vim.loop.fs_scandir(root); if not h then return nil end
  while true do
    local name, typ = vim.loop.fs_scandir_next(h); if not name then break end
    if typ=="file" and name:match("%.json$") then
      if want[name:lower()] then return root.."/"..name end
    end
  end
  return nil
end
...
local function try_load_schema(sig)
  ...
  local candidates = {}
  if r~="" and a~="" and x~="" then table.insert(candidates, string.format("%s-%s-%s-%s", t, r, a, x)) end
  if r~="" and a~=""         then table.insert(candidates, string.format("%s-%s-%s", t, r, a)) end
  if r~=""                   then table.insert(candidates, string.format("%s-%s", t, r)) end
  table.insert(candidates, t)

  for _,root in ipairs({ base, base.."/"..t }) do
    local p = scan_json_candidates(root, candidates)
    if p then
      local js = load_json(p); js = normalize_schema(js)
      if js and type(js.groups)=="table" then return js, p end
    end
  end
  return nil
end
...
local function to_set(lst) local t={}; for _,v in ipairs(lst or {}) do t[v]=true end; return t end
local function build_peer_map(groups)
  local map, all = {}, {}
  for _,g in ipairs(groups or {}) do
    map[g] = to_set(g.starts or {})
    for _,s in ipairs(g.starts or {}) do all[s]=true end
  end
  return map, all
end
...
local function annotate_sg_in_message(buf, node, cfg)
  ...
  if schema == false or not (schema and schema.groups) then return end

  local function tag_of(line) return (line:gsub("^%s+",""):match("^([A-Z][A-Z0-9]+)")) end

  local function annotate_level(start_idx, end_idx, groups)
    if not groups or #groups==0 then return start_idx end
    local peer_map, all_peer_starts = build_peer_map(groups)
    local i = start_idx
    while i <= end_idx do
      local tag = tag_of(lines[i] or "")
      if not tag then i = i + 1 goto continue end

      local g_found = nil
      for _,g in ipairs(groups) do
        if peer_map[g][tag] then g_found = g; break end
      end
      if not g_found then
        i = i + 1
      else
        ...
        while i <= end_idx do
          ...
          if t2 == "UNT" or all_peer_starts[t2] then break end
          if g_found.children and #g_found.children>0 then
        ...
        local base
        if g_found.id and g_found.name then base = g_found.id.." "..g_found.name
        else base = g_found.id or g_found.name or "SG" end
...
local function annotate_pretty(buf)
  ...
  for _,n in ipairs(nodes) do
    ...
    put_label(buf, n.e-1, { { "⟪ /"..(n.type=="interchange" and "Interchange" or n.type=="group" and "Group" or "Message").." ⟫", hl } })
  end
...
local function parse_textish(o) if type(o)=="string" then return o end
  ...
  if o[1]~=nil then
    for _,v in ipairs(o) do
      if type(v)=="table" and ((v["@language"]=="en") or (v["language"]=="en")) and (v["@value"] or v["value"]) then return v["@value"] or v["value"] end
...
local function parse_code(o) if type(o)=="string" or type(o)=="number" then return tostring(o) end
  if type(o)~="table" then return nil end
  if o[1]~=nil then local v=o[1]; if type(v)=="string" or type(v)=="number" then return tostring(v) end
...
local function collect_codes(node,out)
  ...
  for _,v in pairs(node) do if type(v)=="table" then collect_codes(v,out) end end
end
local function jsonld_to_map(txt)
  local ok,obj=pcall(vim.fn.json_decode,txt); if not ok or not obj then return nil end
  local out={}; collect_codes(obj,out)
  if not next(out) and type(obj)=="table" and obj["@graph"] and type(obj["@graph"])=="table" then
    for _,n in ipairs(obj["@graph"]) do collect_codes(n,out) end
  end
...
local function fetch_url(url,cb)
  ...
  local job=vim.fn.jobstart({"curl","-fsSL",url},{
    ...
    on_stdout=function(_,d,_) if d then table.insert(out,table.concat(d,"\n")) end end,
    on_stderr=function() end,
    on_exit=function(_,code) cb(code==0, table.concat(out,"")) end
  })
    ...
    if ok and body and #body>0 then
      ...
      if map and next(map) then
        if save_map(id,base,map) then
          fetcher.saved=fetcher.saved+1; fetcher.last_saved=id; fetcher.last_url=url
          local c=0; for _ in pairs(map) do c=c+1 end; fetcher.last_count=c
          vim.schedule(function() vim.notify(string.format("edi: saved UNCL %04d (%d)",id,c), vim.log.levels.INFO) end)
          done(true); return
        end
      end
    end
  ...
  while fetcher.active<fetcher.pool and fetcher.next_id<=fetcher.max_id do
    ...
    fetch_one(id, state.cfg.data_dir, function()
      ...
      if fetcher.done>=fetcher.max_id and fetcher.active==0 then
        fetcher.running=false; if timer then timer:stop(); timer:close(); timer=nil end
        vim.schedule(function()
          vim.notify("edi: EDIFACT fetch complete. Saved "..fetcher.saved, vim.log.levels.INFO)
          try_load_data_dir(); vim.notify("edi: data reloaded", vim.log.levels.INFO)
        end)
      end
    end)
  end
...
local function cmd_sg_which()
  ...
  if m and m.path then
    vim.notify(("edi: SG schema %s (type=%s, rel=%s)"):format(m.path, m.type or "?", m.release or "?"), vim.log.levels.INFO)
    return
  end
  local buf = vim.api.nvim_get_current_buf()
  local nodes, cfg = build_tree(buf)
  local sig = nil
  for _,n in ipairs(nodes) do if n.type=="message" then sig = n.sig or parse_message_sig(vim.api.nvim_buf_get_lines(buf, n.s-1, n.s, false)[1] or "", cfg); break end end
  if not sig then vim.notify("edi: no UNH/S009 signature found", vim.log.levels.WARN); return end
  local js, path = try_load_schema(sig or {})
  if js and path then
    vim.notify(("edi: candidate SG schema %s (type=%s, rel=%s)"):format(path, sig.type or "?", sig.release or "?"), vim.log.levels.INFO)
  else
...
local function cmd_sg_dump()
  ...
  local nodes, cfg = build_tree(buf)
  local msg = nil
  for _,n in ipairs(nodes) do if n.type=="message" then msg = n; break end end
  if not msg then return vim.notify("edi: no message node found", vim.log.levels.WARN) end

  local sig = msg.sig or parse_message_sig((vim.api.nvim_buf_get_lines(buf, msg.s-1, msg.s, false)[1] or ""), cfg)
  local js, path = try_load_schema(sig or {})
  if not js then return vim.notify("edi: no schema found", vim.log.levels.WARN) end
  js = normalize_schema(js)
  if not js or not js.groups then return vim.notify("edi: schema has no groups after normalization", vim.log.levels.WARN) end

  local function rec(gs, d)
    for _,g in ipairs(gs) do
      print(string.rep("  ", d) .. (g.id or "SG?") ..
        " starts=[" .. table.concat(g.starts or {}, ",") .. "]  " .. (g.name or ""))
